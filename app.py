...
def is_duplicate_signal(signal, price):
    """Return True if this signal+price was already processed within TTL window."""
...
# Legacy unprefixed signals — all route to Gold
# FIX: added "NPR_BULL" and "NPR_BEAR" which are the fallback names the NPR
#      Pine Script sends when no specific event type is detected.
LEGACY_GOLD_SIGNALS = {
    "NPR_BULL_ELEPHANT",  "NPR_BEAR_ELEPHANT",
    "NPR_BULL_TAIL",      "NPR_BEAR_TAIL",
    "NPR_BULL_180",       "NPR_BEAR_180",
    "NPR_BULL",           "NPR_BEAR",           # ← ADDED: NPR fallback names
...
def determine_instrument(signal):
    """
    Route signals to the correct instrument:
      MES_ prefix           → MES
      GOLD_ prefix          → Gold
      Legacy unprefixed set → Gold
      Anything else         → None (caught by validation)
    """
...
def determine_direction(signal):
    """
    BULL or BULLISH anywhere in signal → buy (long)
    BEAR or BEARISH anywhere in signal → sell (short)
    """
    su = signal.upper()
    if "BULL" in su:
        return "buy"
...
def health_check():
    return jsonify({"status": "healthy", "message": "Trading Bot is running"}), 200
...
def webhook():
    try:
        ...
        # ── 2. Extract price — accept "price" OR "entry" (NPR scripts use "entry")
        ...
        if not instrument:
            logger.error(f"Cannot determine instrument for: {signal}")
            return jsonify({"error": "Cannot determine instrument"}), 400

        # ── 6. Set per-instrument parameters ─────────────────────────────────
        if instrument == "GOLD":
            ticker                  = MGC_CONTRACT
        ...
        if not action:
            logger.error(f"Cannot determine direction for: {signal}")
            return jsonify({"error": "Could not determine direction"}), 400

        # ── 8. Build TradersPost payload ─────────────────────────────────────
        tp_payload = {
            "ticker":   ticker,
            "action":   action,
            "price":    price,
            "quantity": quantity,
            "takeProfit": {"amount": tp_points},
            "stopLoss":   {"amount": sl_points},
        }
        ...
        if webhook_url:
            try:
                tp_resp = requests.post(webhook_url, json=tp_payload, timeout=10)
        ...
        if trade_success:
            ...
            msg = (
                ...
                f"<b>Price:</b> {price}\n"
                f"<b>TP:</b> {tp_points} pts/contract "
                f"(${tp_dollars_per_contract:.2f}/contract, "
                f"${tp_dollars_per_contract * quantity:.2f} total)\n"
                f"<b>SL:</b> {sl_points} pts/contract "
                f"(${sl_dollars_per_contract:.2f}/contract, "
                f"${sl_dollars_per_contract * quantity:.2f} total)"
            )
